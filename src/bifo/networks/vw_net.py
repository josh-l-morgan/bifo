...
class conVGGBlock(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels,con_shape, dropout_p):
        super().__init__()
        padding = np.array(con_shape)//2
        padding_mode = 'zeros'
        self.relu = nn.ReLU(inplace=True)
        self.dropout1 = nn.Dropout3d(p=dropout_p)
        self.conv1 = nn.Conv3d(in_channels, middle_channels, con_shape, padding=padding, padding_mode=padding_mode)
        self.bn1 = nn.BatchNorm3d(middle_channels)
        self.dropout2 = nn.Dropout3d(p=dropout_p)
        self.conv2 = nn.Conv3d(middle_channels, out_channels, con_shape, padding=padding, padding_mode=padding_mode)
...
class NestedUNet(nn.Module):
    def __init__(self, num_classes=1, input_channels=1, deep_supervision=True, 
                 nb_filter=[16, 32, 64, 128], is_iso=(0,0,1,1), dropout_p=0, **kwargs):
        super().__init__()
        ...
        # Encoder
        self.conv0_0 = conVGGBlock(input_channels, nb_filter[0], nb_filter[0], con_shape[0], dropout_p)
        self.conv1_0 = conVGGBlock(nb_filter[0], nb_filter[1], nb_filter[1], con_shape[1], dropout_p)
        self.conv2_0 = conVGGBlock(nb_filter[1], nb_filter[2], nb_filter[2], con_shape[2], dropout_p)
        self.conv3_0 = conVGGBlock(nb_filter[2], nb_filter[3], nb_filter[3], con_shape[3], dropout_p)

        # Decoder (Nested)
        self.conv0_1 = conVGGBlock(nb_filter[0] + nb_filter[1], nb_filter[0], nb_filter[0], con_shape[0], dropout_p)
        self.conv1_1 = conVGGBlock(nb_filter[1] + nb_filter[2], nb_filter[1], nb_filter[1], con_shape[1], dropout_p)
        self.conv2_1 = conVGGBlock(nb_filter[2] + nb_filter[3], nb_filter[2], nb_filter[2], con_shape[2], dropout_p)

        self.conv0_2 = conVGGBlock(nb_filter[0]*2 + nb_filter[1], nb_filter[0], nb_filter[0], con_shape[0], dropout_p)
        self.conv1_2 = conVGGBlock(nb_filter[1]*2 + nb_filter[2], nb_filter[1], nb_filter[1], con_shape[2], dropout_p)

        self.conv0_3 = conVGGBlock(nb_filter[0]*3 + nb_filter[1], nb_filter[0], nb_filter[0], con_shape[0], dropout_p)
    ...
    def forward(self, input):
        ...
        if self.deep_supervision:
            ...
            return [output1, output2, output3], x0_3
...
class threeScaleNestedUNet(nn.Module):
    def __init__(self, tr ):
        ...
        # Learnable scalar for transfering channels between networks   
        if num_v > 1:
            start_gain = [1 / 5 / (n-1) for n in tr['input_channels'][1:]]
            self.skip_gain = nn.ParameterList([nn.Parameter(torch.tensor(sg))
                                               for sg in start_gain])    
        # ## make dummy network for calculating the validity of voxels    
        # self.m = nn.ModuleList()
        # for v in self.v:
        #     self.m.append(copy.deepcopy(v))
        # with torch.no_grad():
        #     for m in self.m:
    ...
    def forward(self, input_tensor, use_v=None):
        ...
        if use_v is None:
            ...
            out = []
            feats = []
            feat_core = [] ## lists for cutting center out of output for next phase
            feat_core_u = [] ## list for upsampled centers
            for t in range(num_tensor):
                out_chan, feat = self.v[t](in_tensor)
                out.append(out_chan) ## run model on input tensor
                if (num_tensor > 1) & (t < (num_tensor-1)):
                    feats.append(feat[:,0:self.p['input_channels'][t+1]-1,:,:,:])
                    feat_core.append(crop_center(feats[t], self.p['pull'][t]))
                    feat_core_u.append(upsample_tensor(feat_core[t], self.p['ds'][t]))
                    g = self.skip_gain[t]
                    in_tensor = torch.cat([input_tensor[t+1], g * feat_core_u[t]], dim=1)
            return out , feats # full supervision or final output
        
        else:   
            out, feat =  self.v[use_v](input_tensor[0]) ## run model on input tensor
            return out, feat
