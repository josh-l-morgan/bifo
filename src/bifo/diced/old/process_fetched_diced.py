...
class showInferenceLive:
    ...
    # def set_fig_info(self, figi):
        
    #     self.valid = {
    #         'zrms':figi['zrms'],
    #         'corner_1':np.array(figi['corner_1'],int),
    #         'corner_2':np.array(figi['corner_2'],int),
    #         'center': np.array(figi['center'],int)
    #     } 
...
def vast_to_map_request(map_request,
    ...
    fov_corner_1 = fov_center - (np.array(check_size) * use_mip / 2) # find lower corner

    map_request["mip_level"] = use_mip
...
def  run_process(paths, map_request, process_request):
    ...
    mid_z = int(np.floor(fov_center[0]/2))
    ...
    for b in range(num_batch):
        ...
        for c in range(batch_size):
            ...
            if 1:#(c1[cid,0] + mid_p ) == fov_center[0]:
                zrm_em[c1[cid,1]:c2[cid,1]+1,c1[cid,2]:c2[cid,2]+1] = batch[c, 0, mid_c, clp[1]:-clp[1], clp[2]:-clp[2]]
        t[3,1] = time.time()
...
if __name__ == '__main__':
    ...
    run_process(paths, map_request, process_request)
